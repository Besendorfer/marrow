...
The prioritized product and engineering roadmap lives in [`docs/roadmap.md`](docs/roadmap.md). Consult it before choosing substantial new work and keep its status current as items ship or change.
...
- **Chat action protocol is triplicated**: `CHAT_UI_ACTIONS` (`crates/core/src/chat.rs`), the `ChatAction` union (`app/src/types.ts`), and `isChatAction` (`app/src/components/RichText.tsx`) must stay in sync by hand — sync markers at each site. The `marrow-card` answer-card protocol (issue #166) is triplicated the same way, across its own three sites: `CHAT_ANSWER_CARDS` (`crates/core/src/chat.rs`), the `ChatCard` union (`app/src/types.ts`), and `isChatCard` (`app/src/components/ChatCards.tsx`). The `marrow-tool` read-only repo-tools protocol (issue #150) is triplicated the same way too: `CHAT_REPO_TOOLS` (`crates/core/src/chat.rs`), the `ChatToolCall` union (`app/src/types.ts`), and `isChatToolCall` (`app/src/components/RichText.tsx`) — but execution is backend-only (`crates/core/src/chat_agent.rs`), the frontend only renders the fence as a chip.
